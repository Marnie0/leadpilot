/**
 * Everything the seed scripts share: the Prisma handle, the deterministic
 * random source, and the builders that produce an organisation and a lead with
 * a plausible history.
 *
 * Two scripts use this. `seed.ts` builds the demo template and is the only one
 * that ever runs against production. `seed-isolation-tenant.ts` builds a
 * second, ordinary workspace for proving cross-tenant isolation locally, and
 * refuses to run anywhere that is not a development database — see the guards
 * at the top of that file for why it is a separate script at all.
 */
import {
  ActivityType,
  FollowUpChannel,
...
export const pick = <T>(items: readonly T[]): T => items[Math.floor(random() * items.length)] as T;
...
export const STAGE_DISTRIBUTION: Array<{
  key: StageKey;
  count: number;
...
export async function createOrganization(
  ...
  const organization = await prisma.organization.create({
    data: {
      ...
      // `aiEnabled` is deliberately left at its default of false, template
      // included. A visitor's sandbox does not inherit it — `createDemoSandbox`
      // switches the assistant on for every clone — so setting it here would be
      // a flag that reads as though it mattered and does not. No analyses are
      // cloned either: every sandbox starts empty, so a visitor sees the
      // feature run rather than a result somebody else generated.
      isDemo: false,
      stages: {
...
export type SeededOrg = Awaited<ReturnType<typeof createOrganization>>;
